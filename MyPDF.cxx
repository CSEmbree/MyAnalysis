...
MyPDF::MyPDF(string _steeringFile, int iflpdf, double Q2value, TH1D* h1)
{
 //
 ...
 if (FileExists(steeringFileName)==true) {
  ...
  hpdf=h1;
  if (!hpdf) cout<<" MyPDF histograme not found "<<endl;
 
  ifl=iflpdf;

  Q2=Q2value;

  cout<<" MyPDF: Get PDF for Q2= "<<Q2<<" ifl= "<<ifl<<endl;
  if (debug) {
    //cout<<" MyPDF: Get PDF for Q2= "<<Q2<<" ifl= "<<ifl<<endl;
   //cout<<" MyPDF Histogram: "<<endl;
   //hpdf->Print("all");
  }
...
MyPDF::MyPDF(string _gridName, string _steeringFile, bool do_PDFBand, bool do_AlphaS)
{

 debug=false;

 if (debug) cout<<" MyPDF::MyPDF: Start overloaded constructor"<<endl;
 ...
 if (debug)
  cout<<"Setting up MyPDF with:<<<<<<<<<<<<<<<<<<<<<<<<<<"
      <<"\n\t Debug: "<<(debug? "ON":"OFF")
      <<"\n\t GridName: "<<gridName
    //<<"\n\t SteeringFileDir: "<<steeringFileDir
      <<"\n\t SteeringFile: "<<steeringFileName
      <<"\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
 ...
 if(FileExists(steeringFileName)==true) {
  ...
  cout<<" MyPDF::MyPDF WARNING: Couldn't find file names: "<<steeringFileName<<endl;
 //SetVariablesDefault();
   exit (0);
 }

 if (debug)cout<<" MyPDF::MyPDF: end constructor"<<std::endl;
}

// perform any additional work after constructors but before the object is available for use
void MyPDF::Initialize()
{
 if (debug) cout<<" MyPDF::Initialize: Performing Initialization"<<endl;

 //if(steeringFilePath.size()>0) SetSteeringFileNameAndDir(steeringFilePath);
 //else cout<<" MyPDF::Initialize: ERROR: No steering file was provided!"<<std::endl;
 ...
 if ( !do_PDFBand && !do_AlphaS) {
  cout<<" MyPDF::Initialize: ERORR: All theory uncertainties disabled. Possible steering file error? Check file: "<<steeringFileName<<endl;
   exit(0); //TEST
 }

 if (debug) cout<<" MyPDF::Initialize: gridName= "<<gridName.c_str()<<endl; 

 if (gridName.size()==0) {
  //cout<<" MyPDF no applgrid found gridname= "<<TString(gridName).Data()<<endl;
  applgridok=false;
 } else {
  my_grid = new appl::grid(gridName.c_str());

  if (!my_grid) {
   if (debug) cout<<" MyPDF no applgrid found gridname= "<<TString(gridName).Data()<<endl;
   applgridok=false;
  } else {
    if (debug) cout<<" MyPDF applgrid ok gridname= "<<TString(gridName).Data()<<endl;
   applgridok=true;
  }
 }
 ...
 if (debug) 
  cout<<" MyPDF::Initialize: Fill PDF errors for PDFType: "
      <<PDFtype<<", PDFName: "<<PDFname<<", from PDFPath: "<<pdfSetPath<<endl;

 default_pdf_set_name = (std::string) (pdfSetPath+"/"+PDFname+".LHgrid");

 cout<<" MyPDF::Initialize: init PDF set called: "<<default_pdf_set_name.c_str()<<endl;
...
*/
 ...
 if (do_AlphaS) {
  ...
  if (AlphaSmemberNumUp==DEFAULT) {
   cout<<" MyPDF::Initialize: ERROR: 'AlphaSmemberNumUp' not provided in steer file: "<<steeringFileName<<endl;
   exit(0);
  }

  if (AlphaSPDFSetNameUp.compare("")==0) {
   cout<<" MyPDF::Initialize: ERROR: 'AlphaSPDFSetNameUp' not provided in steer file: "<<steeringFileName<<endl;
   exit(0);
  }

  if (AlphaSPDFSetNameDown.compare("")==0) {
   cout<<" MyPDF::Initialize: ERROR: 'AlphaSPDFSetNameDown' not provided in steer file: "<<steeringFileName<<endl;
   exit(0);
  }

  if (AlphaSPDFSetHistNameUp.compare("")==0) {
   cout<<" MyPDF::Initialize: ERROR: 'AlphaSPDFSetHistNameUp' not provided in steer file: "<<steeringFileName<<endl;
   exit(0);
  }

  if(AlphaSPDFSetHistNameDown.compare("")==0) {
   cout<<" MyPDF::Initialize: ERROR: 'AlphaSPDFSetHistNameDown' not provided in steer file: "<<steeringFileName<<endl;
   exit(0);
  }
  ...
  if (applgridok) {
   ...
   if (debug) cout<<" MyPDF::Initialize histo from PDF not grid ! "<<endl;
   temp_hist=this->FillPdfHisto();
   cout<<"  MyPDF::Initialize  hpdf= "<<hpdf->GetName()<<endl;
   //temp_hist->SetName((TString) ("h_pdf_default"));
   temp_hist->SetName(hpdf->GetName());
  }

  if (!temp_hist) cout<<" MyPDF::Initialize temp_hist not found ! "<<endl;

  if (debug) temp_hist->Print("all");

  h_errors_AlphaS.push_back(temp_hist);

  if (debug) cout<<" MyPDF::Initialize Setting up alphas down "<<endl;
  ...
  if (debug) cout<<" MyPDF::Initialize Setting up alphas up "<<endl;
 ...
 if (do_PDFBand) {
  ...
  for (int pdferri = 0; pdferri < n_PDFMembers; pdferri++) {
   if (debug) cout<<" MyPDF::Initialize: pdferri: "<<pdferri<<" of "<<n_PDFMembers
                  <<" pdftype="<<PDFtype.c_str()<<endl;                 
 /*
   if (PDFtype.compare("HERAPDF15NLO")==0){ 
    if (debug) cout<<" MyPDF::Initialize: initPDFSet HERAPDF pdferri= "<<pdferri<<endl;
    if (pdferri <= 20 ) {
...
*/
   if (TString(PDFtype).Contains("HERA") || TString(PDFtype).Contains("ATLAS") ){ 
    ...
    if (debug) cout<<" MyPDF::Initialize: init normal PDF set= "<<pdferri<<endl;
    LHAPDF::initPDF(pdferri);
   }
   ...
   if (applgridok) {
    ...
    if (!tmp) cout<<" MyPDF::Initialize  tmp histogram not found "<<endl;
    temp_hist=(TH1D*)tmp->Clone(tmp->GetName());
    temp_hist->SetName(tmp->GetName());
   }

   if (!temp_hist) cout<<" MyPDF::Initialize  temp_hist histogram not found "<<endl;
   if (pdferri==defaultpdfid) hdefault=(TH1D*)temp_hist->Clone(temp_hist->GetName());

   if (debug) {
    if (pdferri==defaultpdfid)  {
     cout<<" MyPDF::Initialize print defaultpdf histo: "<<endl;
     temp_hist->Print("all");
    } else {
     cout<<" MyPDF::Initialize ratio pdferri=  "<<pdferri<<" to default= "<<defaultpdfid<<endl;
     TString hname=TString("hratio_")+temp_hist->GetName()+TString("pdf_");
     hname+=pdferri;
     hname+="_Divided_by_pdf_";
     hname+=defaultpdfid;
     cout<<" MyPDF::Initialize hname=  "<<hname<<endl;
     if (!hdefault) cout<<" MyPDF::Initialize  hdefault histogram not found "<<endl;
 ...
 if (debug) {
  if (do_AlphaS) {
   cout<<" MyPDF h_AlphaS_results "<<endl;
   h_AlphaS_results->Print("all");
  }
  if (do_PDFBand) {
   cout<<" MyPDF::h_PDFBand_results: "<<endl;
   h_PDFBand_results->Print("all");
   cout<<" MyPDF:: default= "<<defaultpdfid<<endl;
   hpdfdefault->Print("all");
   /*
   if (debug) {
    cout<<" MyPDF::Initialize After syst uncertainty calculation ratio to default "<<endl;
...
TGraphAsymmErrors *MyPDF::GetScaleDependence(double renscale, double facscale, int pdfset=0){
  
  if (debug)
   cout<<" MyPDF::GetScaleDependence renscale= "<<renscale<<" facscale= "<<facscale<<" PDF= "
       <<default_pdf_set_name.c_str()<<endl;

  LHAPDF::initPDFSet(default_pdf_set_name.c_str(), pdfset);

  if (debug) cout<<" MyPDF::GetScaleDependence  now get histo: "<<endl;
  ...
  TH1D* temp_hist = 0;
   ...
   if (applgridok) {
    ...
    cout<<" MyPDF::GetScaleDependence histo from templates not found ! "<<endl;
    //temp_hist->SetName((TString) ("h_pdf_default"));
   }

  if (!temp_hist) cout<<" MyPDF::GetScaleDependence  temp_hist not found "<<endl;
  ...
  gtmp->SetName(gname);
  if (debug) {
   cout<<" MyPDF::GetScaleDependence got: "<<gtmp->GetName()<<endl;
   gtmp->Print("all");
  }
...
{
 if (debug)
  cout<<" MyPDF::CalcSystErrors: Start syst error calc for: "<<PDFtype
      <<"\n\tPDFBand: "<<do_PDFBand
  ...
  if (debug) cout<<" MyPDF::CalcSystErrors: End syst error calc for: "<<PDFtype<<endl;
}
...
void MyPDF::CalcPDFBandErrors()
{
 ...
 if (debug) cout<<" MyPDF::CalcPDFBandErrors: Start calc of PDFBandErrors for: "<<PDFtype<<endl;
 if (debug) cout<<" MyPDF::CalcPDFBandErrors: defaultpdf= "<<defaultpdfid<<endl;
 ...
 if (debug) {
  cout<<" MyPDF::CalcPDFBandErrors: defaultpdf: "<<endl;
  hpdfdefault->Print("all");
 }

 for (int bi = 1; bi <= h_errors_PDFBand.at(0)->GetNbinsX(); bi++) { // loop over bins
  if (debug) cout<<" MyPDF::CalcPDFBandErrors: bin= "<<bi<<endl;
  ...
  if (TString(PDFtype).Contains("NNPDF") ) {
   ...
   if (debug)
    cout<<" MyPDF::CalcPDFBandErrors: CT10  this_err_up= "<<this_err_up<<" PDFErrorSize= "<<PDFErrorSize.c_str()<<endl;
   if (PDFErrorSize.compare("OneSigma")==0) {
    if (debug) cout<<" MyPDF::CalcPDFBandErrors: put to one sigma "<<endl;
    this_err_up /= 1.645;
   }
   this_err_down = this_err_up;
   ...
   for (int pdferri = 1; pdferri < (int) h_errors_PDFBand.size()-1; pdferri += 2) {
    ...
    if (debug) cout<<" MyPDF::CalcPDFBandErrors: CT10 pdferri= "<<pdferri
   ...
   if (debug) {
    cout<<" MyPDF::CalcPDFBandErrors: HERA/ATLAS type PDF "<<endl;
    cout<<" MyPDF::CalcPDFBandErrors: PDFname= "<<PDFname.c_str()<<" PDFnamevar= "<<PDFnamevar.c_str()<<endl;
    cout<<" MyPDF::CalcPDFBandErrors: firsteig= "<<firsteig<<" lasteig= "<<lasteig<<endl;
    cout<<" MyPDF::CalcPDFBandErrors: firstquadvar= "<<firstquadvar<<" lastquadvar= "<<lastquadvar<<endl;
    cout<<" MyPDF::CalcPDFBandErrors: firstmaxvar= "<<firstmaxvar<<" lastmaxvar= "<<lastmaxvar<<endl;

    cout<<" MyPDF::CalcPDFBandErrors: defaultpdfid= "<<defaultpdfid<<" defaultpdfidvar= "<<defaultpdfidvar<<endl;
   }

   central_val = h_errors_PDFBand.at(defaultpdfid)->GetBinContent(bi);

   if (debug) cout<<" MyPDF::CalcPDFBandErrors: default sample id= "<<defaultpdfid<<" "<<" central_val= "<<central_val<<endl;
   ...
   if (includeEIG) {
    for (int pdferri = firsteig; pdferri <=lasteig; pdferri += 2) {   //// experimental errors
     ...
     if (debug) cout<<" MyPDF::CalcPDFBandErrors: "<<PDFtype.c_str()
   ...
   if (debug) cout<<" MyPDF::CalcPDFBandErrors: variation sample id= "<<firstvar+defaultpdfidvar<<" "<<" central_val= "<<central_val<<endl;
   if (includeQUAD) {
   ...
   if (includeMAX) {
    for (int pdferri = firstvar+firstmaxvar; pdferri < firstvar+lastmaxvar; pdferri++) { // model errors
     ...
     //if (debug) cout<<" MyPDF::CalcPDFBandErrors: diff_central= "<<diff_central
     //               <<" var/nom= "<<h_errors_PDFBand.at(pdferri)->GetBinContent(bi)/central_val
     //               <<endl;
     ...
     if (debug) cout<<" MyPDF::CalcPDFBandErrors: "<<PDFtype.c_str()<<" pdferri= "
...
*/
    ...
    cout<<" MyPDF::CalcPDFBandErrors: unsupported pdfCode encountered."<<PDFtype.c_str()<<endl;
   exit(0); //TEST
  }
  ...
  if (debug) {
   ...
   double y_val;
   h_PDFBand_results->GetPoint(bi-1, x_val, y_val);
   //cout<<" MyPDF::CalcPDFBandErrors: y_val= "<<y_val<<endl;
   //cout<<" MyPDF::CalcPDFBandErrors: x_val= "<<x_val<<endl;
   if (y_val!=0) {
    cout<<" MyPDF::CalcPDFBandErrors: bin= "<<bi<<" (y_val+this_err_up)/y_val= "  <<(y_val+this_err_up)/y_val
                                                <<" (y_val-this_err_down)/y_val= "<<(y_val-this_err_down)/y_val<<endl;
   }
  }
 }  /// loop over bins

 if(debug) cout<<" MyPDF::CalcPDFBandErrors: End cal of PDFBandErrors for: "<<PDFtype<<std::endl;
}

void MyPDF::CalcAlphaSErrors()
{
 if (debug) cout<<" MyPDF::CalcAlphaSErrors: Starting calc of PDFAlphaSErrors for: "<<PDFtype<<std::endl;
 ...
 if (!do_AlphaS) {
  cout<<" MyPDF::CalcAlphaSErrors: called but do_AlphaS off ! "<<endl;
  return;
 }
...
//if (debug) cout<<" Size h_errors_AlphaS= "<<h_errors_AlphaS.size()<<endl;
 ...
 if (h_errors_AlphaS.size()==0)
  cout<<" MyPDF::CalcAlphaSErrors: h_errors_AlphaS not filled "<<endl;
 if (!h_errors_AlphaS[0])
  cout<<" MyPDF::CalcAlphaSErrors: h_errors_AlphaS[0] not found ! "<<endl;

 for (int bi = 1; bi <= h_errors_AlphaS.at(0)->GetNbinsX(); bi++) {
  //if (debug) cout<<" MyPDF::CalcAlphaSErrors: bin= "<<bi<<endl;
  ...
  if (debug) cout<<" MyPDF::CalcAlphaSErrors: bin = "<<bi
 ...
 if (debug) cout<<" MyPDF::CalcAlphaSErrors: End calc of PDFAlphaSErrors for: "<<PDFtype<<std::endl;
}


void MyPDF::GetRatioToTH1(TH1D* href)
{
 if (debug) cout<<" MyPDF::GetRatioToTH1: start"<<endl;
 ...
 if (debug) cout<<" MyPDF::GetRatioToTH1: End"<<endl;
}
...
void MyPDF::DrawPDFRatio(int iset1, int iset2){
...
double MyPDF::GetMaximum(int iset){

  return h_errors_PDFBand[iset]->GetMaximum();
}

TH1D* MyPDF::GetPDFRatio(int iset1, int iset2)
{

 if (debug) 
  cout<<" MyPDF::GetPDFRatio iset2= "<< iset2<<" iset1= "<<iset1
 ...
 if (iset1>=n_PDFMembers||iset2>=n_PDFMembers||iset1<0||iset2<0){
  cout<<" MyPDF::GetPDFRatio iset too large iset2= "<< iset2<<" iset1= "<<iset1<<endl;
  exit (0);
 }
 ...
 if (debug) cout<<" MyPDF::GetPDFRatio: "<< ratio_to_ref_name.Data()<<endl;
...
double MyPDF::GetPDFWeight(int iset1, double x1, double x2){
 double w=1.;
...
{
 //if(debug) cout<<" MyPDF::TH1TOTGraphAsymm: start"<<std::endl;
 ...
 //if(debug) cout<<" MyPDF::TH1TOTGraphAsymm: End"<<std::endl;
 return g1;
}
...
TGraphAsymmErrors* MyPDF::MyTGraphErrorsDivide(TGraphAsymmErrors* g1,TGraphAsymmErrors* g2, Int_t noerr) {
 if (debug) cout<<" MyPDF::MyTGraphErrorsDivide: started"<<std::endl;
 if (!g1) cout<<" MyPDF::MyTGraphErrorsDivide: g1 does not exist ! "<<std::endl;
 if (!g2) cout<<" MyPDF::MyTGraphErrorsDivide: g2 does not exist ! "<<std::endl;

 Int_t n1=g1->GetN();
 Int_t n2=g2->GetN();

 if (n1!=n2) {
  cout<<" MyPDF::MyTGraphErrorsDivide: vector do not have the same number of entries!"
      <<"\n\tg1: "<<g1->GetName()<<" n1= "<<n1
      <<"\n\tg2: "<<g2->GetName()<<" n2= "<<n2<<endl;
  }

  TGraphAsymmErrors* g3= new TGraphAsymmErrors();
  if (!g3) cout<<" MyPDF::MyTGraphErrorsDivide: problem to make new vector ! "<<std::endl;
  ...
  for (Int_t i1=0; i1<n1; i1++) {  //loop over point of graph1
   ...
   for (Int_t i2=0; i2<n2; i2++) {//loop over point of graph2
    ...
    if (fabs(x1-x2)>emean) {
     //cout<<" MyPDF::MyTGraphErrorsDivide: x1 and x2 not the same x1= "<<x1<<" x2= "<<x2<<std::endl;
    } else { // do something only if x1=x2
     matchcount++;
     //cout<<" MyPDF::MyTGraphErrorsDivide: x1 and x2 match x1= "<<x1<<" x2= "<<x2<<std::endl;
     dx1h  = EXhigh1[i1];
     dx1l  = EXlow1[i1];
     ...
     if (debug) {
      cout<<"MyPDF::MyTGraphErrorsDivide: "
          <<"\n\ti1: "    <<i1<<", i2: "<<i2
          <<"\n\tdy1l: "  <<dy1l<<", dy1h: "<<dy1h
          <<"\n\tdy2l: "  <<dy2l<<", dy2h "<<dy2h
    ...
    if (matchcount>1) {
     cout<<" MyPDF::MyTGraphErrorsDivide: too many x-points matched ! "<<endl;
     exit (1);
    }
   }

   if (debug) cout<<" MyPDF::MyTGraphErrorsDivide: End"<<std::endl;
   return g3;
}
...
void MyPDF::ReadSteering(const string _fileName)
{
 ...
 if (debug) cout<<" MyPDF::ReadSteering: reading steering file named: "<<fName.c_str()<<std::endl;
 ...
 if (pdfSetDefaultPath.size()>0) {
  ...
  cout<<" MyPDF: LHAPATH environment variable found, using path: "<<pdfSetPath<<endl;
 } else {
  cout<<" MyPDF: LHAPATH environment variable not set, using default: "<<pdfSetPath<<endl;
 }
 ...
 while (infile.good()) {
  ...
  if (line[0] != '%') { //ignore comments
   if (optionName.compare("debug")==0) {
    ...
    cout<<" MyPDF::ReadSteering: new PDF set path found! Now path is: "<<pdfSetPath<<std::endl;
...
{
 if (debug) cout<<" MyPDF::setVariablesDefault: Start default values being set."<<std::endl;
 ...
 if (debug) cout<<" MyPDF::setVariablesDefault: End default values are set."<<std::endl;
}
...
{
    if(debug) cout<<" MyPDF::setSteeringFileNameAndDir: Start extracting file name and directory name from path: "<<_path<<std::endl;
    ...
    if(pathLength>0) {
        ...
        if(debug) {
            cout<<" MyPDF::setSteeringFileNameAndPath:"
                     <<" \n\tSplitting: '"<<_path<<"'"
        ...
        cout<<" MyPDF::setSteeringFileNameAndDir: Steering file path '"<<_path<<"' is invalid."<<std::endl;
    }
}
*/
...
void MyPDF::CleanUpMyPDF() {
 if (debug) cout<<"MyPDF::CleanUpMyPDF: Starting to clean up..."<<std::endl;
 ...
 if (h_errors_AlphaS.size()>0) {
  ...
  if (debug) cout<<"MyPDF::CleanUpMyPDF: Finished clean up!"<<std::endl;
}



TH1D * MyPDF::FillPdfHisto(){

  if (!hpdf) cout<<" MyPDF::FillPdfHistos histo not found "<<endl;
  ...
  if (debug) cout<<" MyPDF::FillPdfHisto: xmin= "<<xmin<<" xmax= "<<xmax<<" binw= "<<binw<<endl;
  if (debug) cout<<" MyPDF::FillPdfHisto: Q2= "<<Q2<<endl;
  ...
  for (int i=0; i<nbin; i++){
   ...
   if (debug) 
    cout<<" MyPDF::FillPdfHisto: ibin= "<<ibin<<" Q2= "<<Q2<<" x= "<<x<<" xfl["<<ifl<<"]= "<<xfl[6+ifl]<<endl;
  ...
  /*
  if (debug) {
   cout<<" MyPDF::FillPdfHisto print histo: "<<endl;
   hpdf->Print("all");
  }
  */
  return hpdf;
