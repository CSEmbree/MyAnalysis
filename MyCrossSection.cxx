...
void MyCrossSection::DrawinFrame(int iframe) {
 ...
 // if (debug) cout<<" MyCrossSection::DrawinFrameFile now DrawFrame  "<<myframe<<endl;
 ...
 if (npdf<1) {
  ...
  for (int ipdf=0; ipdf<npdf; ipdf++) {
   if (debug) cout<<" MyCrossSection::DrawinFrame: label= "
		...
		//<<TString(myband.at(igrid)->GetPdfBand(ipdf)->GetName()).Data() <<endl;
                //<<TString(t_mypdf.at(igrid).at(i)->GetPDFtype()).Data() <<endl;

   //int mysize=TString(t_mypdf.at(igrid).at(i)->GetPDFtype()).Sizeof(); 
   //int mysize=TString(myband.at(igrid)->GetPdfBand(ipdf)->GetName()).Sizeof(); 
 ...
 if (debug) cout<<" MyCrossSection::DrawinFrame: namesize= "<<namesize<<endl;
 // x1-=namesize*0.025;
 x2+=namesize*0.02;

 if (debug) cout<<" MyCrossSection::DrawinFrame: x1= "<<x1
 ...
 for (int i=0; i<(int)gridinframe[iframe].size(); i++) {
  ...
  for (int ipdf=0; ipdf<npdf; ipdf++) {
   ...
   if (ipdf==0) leg->AddEntry((TObject*)0, "NLO QCD with:", "");
  ...
  leg->Draw();
  //if (debug) cout<<" MyCrossSection::DrawinFrame: legend prepare "<<endl;
 }
